...
void TestExampleSize() {
    ...
    sheet->SetCell("E7"_pos, "test");
    sheet->SetCell("K15"_pos, "test");
    sheet->ClearCell("E7"_pos);
    std::cout << "sheet->GetPrintableSize(): " << sheet->GetPrintableSize() << std::endl;
    sheet->ClearCell("K15"_pos);
    std::cout << "sheet->GetPrintableSize(): " << sheet->GetPrintableSize() << std::endl;

    ostringstream text_after;
    sheet->PrintTexts(text_after);
    ostringstream text_after_expected {
        "=(1+2)*3\t=1+2*3\t\t\n"s
        "some\ttext\there\t\n"s
        "\t\t'and'\t'here\n"
        "\t\t\t\n"s
    ...
    ASSERT_EQUAL(text_after.str(), text_after_expected.str());

    ostringstream values_after;
    sheet->PrintValues(values_after);
    ostringstream values_after_expected {
        "9\t7\t\t\n"s
        "some\ttext\there\t\n"s
        "\t\tand'\there\n"
        "\t\t\t\n"s
    ...
    ASSERT_EQUAL(values_after.str(), values_after_expected.str());
...
void TestExampleLeak() {
    ...
    sheet->SetCell("D4"_pos, "=A1+B2+C3");
    sheet->PrintValues(std::cout);
