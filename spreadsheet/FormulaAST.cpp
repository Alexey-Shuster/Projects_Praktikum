...
public:
    ...
    double Evaluate(const SheetInterface& sheet) const override {
        double result;
        try {
            auto left_value = lhs_->Evaluate(sheet);
            auto right_value = rhs_->Evaluate(sheet);
            switch (type_) {
            case Add:
            ...
            case Divide:
                ...
                break;
            default:
                throw std::runtime_error("Unknown exception in BinaryOpExpr::Evaluate");
            }
...
public:
    ...
    double Evaluate(const SheetInterface& sheet) const override {
        if (!cell_->IsValid()) {
            throw FormulaError(FormulaError::Category::Ref);  // Cell position out of MAX size
        }
        ...
        if (std::holds_alternative<double>(result)) {
            ...
            throw std::runtime_error("Unknown exception in CellExpr::Evaluate");
        }
