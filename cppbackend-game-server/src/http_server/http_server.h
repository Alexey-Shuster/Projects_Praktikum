...
protected:
    ...
    const beast::tcp_stream& GetStream() {
        return stream_;
    }
...
private:
    ...
    void HandleRequest(HttpRequest&& request) override {
        auto endpoint = GetStream().socket().remote_endpoint();

        // Pass endpoint and request to handler (logging is now in LoggingRequestHandler)
        request_handler_(std::move(endpoint), std::move(request),
...
private:
    ...
    void DoAccept() {
        // Для асинхронного ожидания подключений используется метод tcp::acceptor::async_accept
        // метод:асинхронно дождётся подключения клиента; создаст сокет, передав ему executor;
        // вызовет функцию-обработчик и передаст в неё код ошибки и созданный сокет.
        acceptor_.async_accept(
            // Передаём последовательный исполнитель, в котором будут вызываться обработчики
            // асинхронных операций сокета.
            // Тогда переданный сокет будет вызывать свои функции-обработчики последовательно и независимо от функций-обработчиков других сокетов.
            net::make_strand(ioc_),
            // С помощью bind_front_handler создаём обработчик, привязанный к методу OnAccept текущего объекта.
            ...
            // Этот вызов bind_front_handler аналогичен namespace ph = std::placeholders;
    ...
    void OnAccept(sys::error_code ec, tcp::socket socket) {
        ...
        if (ec) {
            ...
            boost_logger::LogError(EXIT_FAILURE, ec.message(), boost_logger::error_locations::accept);
            return;
        }
    ...
    // желательно как можно быстрее выйти из обработчика и начать ожидать подключение следующего клиента.
    // Поэтому дальнейшую работу с сокетом будем проводить в его собственном strand. -> SessionBase::Run()
